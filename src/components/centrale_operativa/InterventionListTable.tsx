...
export function InterventionListTable() {
  ...
  const handleWhatsAppMessage = useCallback((event: Intervention) => {
  ...
  const handleEmailReport = useCallback(async (event: Intervention) => {
  ...
  const columns: ColumnDef<Intervention>[] = useMemo(() => [
    ...
    {
      accessorKey: 'barcode', // New column for barcode
      header: 'Barcode',
