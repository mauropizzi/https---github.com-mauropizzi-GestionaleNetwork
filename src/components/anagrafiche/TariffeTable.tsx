...
export function TariffeTable() {
  ...
  const columns: ColumnDef<Tariffa>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => `${row.original.client_rate.toFixed(2)} €`,
    },
    {
      accessorKey: "supplier_rate",
      header: "Tariffa Fornitore (€)",
      cell: ({ row }) => `${row.original.supplier_rate.toFixed(2)} €`,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => row.original.data_inizio_validita ? format(new Date(row.original.data_inizio_validita), "PPP", { locale: it }) : "N/A",
    },
    {
      accessorKey: "data_fine_validita",
      header: "Fine Validità",
      cell: ({ row }) => row.original.data_fine_validita ? format(new Date(row.original.data_fine_validita), "PPP", { locale: it }) : "N/A",
    },
    {
      id: "actions",
      header: "Azioni",
