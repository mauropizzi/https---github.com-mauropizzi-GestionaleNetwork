...
import { Personale, PuntoServizio, RegistroDiCantiere } from "@/lib/anagrafiche-data"; // Corrected import
...
interface CantiereFormProps {
  ...
  report?: RegistroDiCantiere | null; // Added for editing
  onSaveSuccess?: () => void; // Callback for successful save/update
  onCancel?: () => void; // Callback for cancel button
}

export function CantiereReportForm({ reportId, report, onSaveSuccess, onCancel }: CantiereFormProps) {
  ...
  const [loadingInitialReport, setLoadingInitialReport] = useState(!!reportId || !!report); // Keep this for report loading
  ...
  useEffect(() => {
    const loadReportData = async () => {
      const currentReportId = reportId || report?.id;
      if (currentReportId && !loadingDropdowns) { // Only proceed if reportId exists and dropdowns are loaded
        setLoadingInitialReport(true);
        const { data: fetchedReport, error: reportError } = await supabase
        ...
        if (fetchedReport) {
          form.reset({
            reportDate: (fetchedReport.report_date && typeof fetchedReport.report_date === 'string') ? parseISO(fetchedReport.report_date) : new Date(),
            reportTime: fetchedReport.report_time || format(new Date(), "HH:mm"),
            servicePointId: fetchedReport.service_point_id || "", // Use the new service_point_id
            addetto: fetchedReport.employee_id || "",
            servizio: fetchedReport.service_provided || "",
            startDateTime: (fetchedReport.start_datetime && typeof fetchedReport.start_datetime === 'string') ? parseISO(fetchedReport.start_datetime) : undefined,
            endDateTime: (fetchedReport.end_datetime && typeof fetchedReport.end_datetime === 'string') ? parseISO(fetchedReport.end_datetime) : undefined,
            noteVarie: fetchedReport.notes || null,
            automezzi: fetchedReport.automezzi_utilizzati || [],
            attrezzi: fetchedReport.attrezzi_utilizzati || [],
            latitude: fetchedReport.latitude || null,
            longitude: fetchedReport.longitude || null,
            addettoRiconsegnaSecurityService: fetchedReport.addetto_riconsegna_security_service || null,
            responsabileCommittenteRiconsegna: fetchedReport.responsabile_committente_riconsegna || null,
            esitoServizio: fetchedReport.esito_servizio || null,
            consegneServizio: fetchedReport.consegne_servizio || null,
            status: fetchedReport.status || "attivo",
          });
        }
        setLoadingInitialReport(false);
      } else if (!currentReportId) { // If no reportId, ensure loading is false
        setLoadingInitialReport(false);
      }
    };

    loadReportData();
  }, [reportId, report, loadingDropdowns, form]); // Add loadingDropdowns as a dependency
  ...
  const handleSaveDraft = async (values: z.infer<typeof formSchema>) => {
    ...
    showSuccess(`Rapporto di cantiere ${currentReportId ? 'modificato' : 'registrato'} con successo!`);
    form.reset();
  ...
  const handleCloseReport = async (values: z.infer<typeof formSchema>) => {
    ...
    showSuccess(`Rapporto di cantiere ${currentReportId ? 'modificato' : 'registrato'} e completato con successo!`);
    form.reset();
    onSaveSuccess?.(); // Call success callback
  };
